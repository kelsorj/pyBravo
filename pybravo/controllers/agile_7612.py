...
if TYPE_CHECKING:
    from pybravo.types import GripperDetectionState
...
# Per-axis servo register 0xA0 values.
...
class Agile7612Controller(AgileController):
    ...
    def _agile_7612_write_home_reg(self, axis: Axis, data: bytes) -> None:
        ...
        The firmware uses header 0x01 (REG_GET_AX0) with data for these registers,
    ...
    def _agile_7612_fault_reset_ctrl2(self) -> None:
        """Post-move fault reset on controller 2."""
        raw = bytearray(10)
        raw[0] = 0x00
        raw[1] = 0x01
        raw[7] = 0x31
    ...
    def _agile_7612_move_go(self, axes: list[Axis]) -> None:
        ...
        Separate triggers are sent per axis (not combined bitmasks),
    ...
    def _agile_7612_home_single_axis(self, axis: Axis) -> None:
        """Home one axis using the 3-phase sequence.
    ...
    # =================================================================
    # Each axis gets its own method so that capture-matched byte sequences
    ...
    def _home_x(self) -> None:
        """Home X axis — capture-matched sequence from x_axis captures.
        ...
        # Pre-homing: read reg 0x60 (required for re-home, not fresh init)
        try:
    ...
    def _home_y(self) -> None:
        """Home Y axis — capture-matched sequence from y_axis captures.
    ...
    def _home_z(self) -> None:
        """Home Z axis — capture-matched sequence from z_axis captures.
        ...
        # This trigger uses controller_base routing (ax=0), not axis.value
        info = self._move_info_cls(
            axis=axis, position=0.0, velocity=vel, acceleration=accel,
    ...
    def _home_w(self) -> None:
        """Home W axis (plunger) — capture-matched sequence from w_axis captures.
    ...
    def _home_g(self) -> None:
        """Home G axis (gripper) — capture-matched sequence from g_axis captures.
        ...
        # Read register 0x10 (required, but G always uses 2-phase).
        # Controller 2 returns different sensor values than Controller 1
        # (0x7C/0x74 instead of 0x7F), and they are ignored for direction.
        try:
            resp = self._agile_7612_ext_read(0x10, axis)
            if len(resp) >= 3:
    ...
    def _home_zg(self) -> None:
        """Home Zg axis — capture-matched sequence from zg_axis captures.
        ...
        # Read register 0x10 (vendor firmware does this but always uses 2-phase for Zg).
        # Controller 2 returns different sensor values than Controller 1.
        try:
            resp = self._agile_7612_ext_read(0x10, axis)
            if len(resp) >= 3:
    ...
    def _home_xy_parallel(self) -> None:
        """Home X and Y with interleaved per-axis commands (vendor pattern).

        The vendor firmware configures and triggers each axis BEFORE configuring the next,
    ...
    def home_axes(self, axes: list[Axis], *, force: bool = False) -> None:
        """Home axes with a capture-matched sequence.

        Ordering: Z first (safety), then Zg, G, X+Y parallel, W last.
        Each axis dispatches to its own _home_* method.
        Fault flags are cleared first (matches the vendor/InitializeTask pattern).
        """

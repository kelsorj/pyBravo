...
Reimplements the vendor Agile controller firmware -- communicates with a
Rabbit microcontroller via serial or TCP, which forwards 10-byte Agile
packets to one or two motor controllers on the Agile bus.
...
if TYPE_CHECKING:
    from pybravo.protocol.agile_packet import AgileReply
...
class AgileController(BravoController):
    ...
    Reimplements the vendor Agile controller firmware.  The Rabbit
    microcontroller sits between the PC and the Agile motor controllers; it
    accepts V11 framed commands and relays 10-byte Agile packets.
    """

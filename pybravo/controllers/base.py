...
The abstract interface that the Agile, Darwin, and Simulation controllers
all implement. Anything in the codebase that drives hardware goes through
this interface, so a new backend only has to satisfy these methods.
"""
...
from pybravo.types import Axis, DeviceStateFlag, GripperDetectionState, SpeedLevel
...
class BravoController(ABC):
    ...
    def home_axes(self, axes: list[Axis], *, force: bool = False) -> None:
        ...
        itself homed. Backends that always home unconditionally may ignore it.
        Use it for an explicit operator "home this axis" request, where doing
        nothing because the axis looks homed is the wrong answer.
        """

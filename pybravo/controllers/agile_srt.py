...
un-homed start; it is pinned by ``tests/test_agile_srt_homing.py``. Jog and the
Agile 7612 sensor-adaptive homers are NOT reused — their move parameters and
servo constants are tuned for different hardware.
"""
...
# off-sensor -> 3-phase (approach, depart overshoot, slow approach). This matches
# the firmware's expectations and the inherited Agile 7612 homing.
#
# The homing-complete (0x52) marker is sent with an empty data field, matching
# the Agile 7612 homing and the SRT's own Z/W markers. The X/Y markers can carry
# scenario-specific data bytes; those are situational and must not be replayed
# from one homing scenario into another, so the data field is left empty.
...
# W pump-parameter pre-config block, sent twice during a cold init. Entries:
# ("reg", register, 7-byte-hex) servo writes, or ("op", byte7) header-0x00
# axis-bitmask operations.
_SRT_W_PUMP_BLOCK = (

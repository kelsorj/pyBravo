...
class SimulationController(BravoController):
    ...
    # ADC values for known head types (per the ADC-to-resistance table)
    ...
    def get_all_positions(self) -> dict[str, float]:
        """Return all axis positions, keyed by axis NAME.

        Name-keyed, matching the hardware controllers. ``Axis`` is an IntEnum,
        so an enum-keyed dict serialises to JSON as ``{"0": ..., "1": ...}``
        and every consumer looking up "X"/"Y"/"Z" silently reads nothing.
        """
        return {Axis(i).name: ax.position for i, ax in enumerate(self._axes)}

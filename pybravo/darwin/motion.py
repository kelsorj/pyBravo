...
from pybravo.darwin.axis import read_motor_state
...
def build_load_packets(
    ...
    the 7-packet pattern the firmware expects. (An initial INSTR_CLEAR was
    tried here but broke event binding — the controller keeps its own
    instruction-slot state across moves; NEW_INSTR(1) is sufficient.)
    """
...
def _make_move_instruction(
    ...
    *,
    ...
    # word3 (trig_at_value) defaults to the target position — real-world
    # MoveAbsolute instructions set word3 == word2. Firing the SEND event
    # depends on the axis reaching this trigger position.
    inst.trig_at_float = (

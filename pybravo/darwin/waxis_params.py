...
57 parameters × 5 head-type sets. Each head-type resolves to a named
...
→ 0x00001388) or the firmware replies with NAK_OUT_OF_RANGE (a float-encoded
5000.0 lands at a huge magnitude when reinterpreted).
...
Application:
    ...
    3. Remember the current head type so subsequent enable/move calls can skip
       the re-apply unless the head changed.
"""
...
# Parameters the axis controller declares as UInt32. Every
...
def param_set_for_head(head_type: HeadType) -> WAxisParamSet | None:
    ...
    Unknown heads return None so the caller skips the apply entirely.
    """

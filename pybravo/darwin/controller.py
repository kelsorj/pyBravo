"""Pure-Python DarwinController.

Subclasses :class:`BravoController` and implements its interface using the
GeminiEngine + darwin.* modules — the controller talks the Gemini wire
protocol directly, with no external helper process.
...
# grip/jog/force-moves can scale from the original max — i.e. whatever value
# the firmware booted with, read once per session.
...
def _motion_timeout_ms(
    ...
    Timeout is travel time plus ``margin_ms``, floored at ``min_ms``. The
    minimum speed clamp (0.1 mm/s) prevents a divide-by-zero for no-op moves.
    """
...
class DarwinController(BravoController):
    ...
    def __init__(
        ...
        *,
        ...
        # State-snapshot cache — shape is fixed by what higher layers
        # (bravo.py, tasks.py) expect from get_state_snapshot().
    ...
    def _post_connect_init(self) -> None:
        ...
        residual bindings from a prior session can interfere with our new
        START_EVT/SEND_EVT values on event 1. Since we reconnect rather than
        run continuously, we have to explicitly reset.
        """
    ...
    def move(
        ...
        for m in moves:
            ...
            from pybravo.protocol.gemini.enums import MotorState as _MS
        ...
        for m in moves:
            ...
            from pybravo.protocol.gemini.enums import InstructionTypes
    ...
    def home_axes(self, axes: list[Axis], *, force: bool = False) -> None:
        ...
        already up. An explicit operator home must pass ``force=True``, or
        clicking Home on a healthy axis does nothing at all.
        """
    ...
    def is_axis_homed(self, axis: Axis) -> bool:
        ...
        This is the axis's initialized state, NOT the raw HOMING_FLAG_STATE
        sensor reading (which tells you whether the physical flag sensor is
    ...
    def reset_faults(self, axes: list[Axis]) -> None:
        # Effectively a no-op on DARWIN — there is no fault latch to clear.
        for a in axes:
    ...
    def recover(self, axes: list[Axis] | None = None) -> dict[Axis, str]:
        ...
        event). Recovery sequence is: SAFETY_STATUS check → SET
        MOTOR_STATE=ENABLE per axis.
    ...
    def detect_smart_head(self) -> bool:
        ...
        Sends SMART_INIT=0 to the master. Success means a smart head
        responded; NAK ``UNSUCCESSFUL_OPERATION`` means no smart head is
        present.
        """
        from pybravo.protocol.gemini.enums import CommandNAKTypes
    ...
    def read_head_identification(self) -> dict:
        ...
        The EEPROM byte is the value at smart-head offset 1 — the firmware's
        head-type byte. ADC counts (``STUPID_HEAD_COUNTS``) are useful for
        resistor-based heads or as a cross-check.
    ...
    def detect_gripper(self) -> GripperDetectionState:
        # Gripper presence is proven by reading FirmwareVersion from the
        # sub-node that owns the Zg device. Success means the gripper
        # sub-node is alive on the bus; failure (timeout or NAK) means no
        # gripper is attached. In our stack the gripper lives at
        # InstructionAddress(6); a successful FW_VERSION read is a
    ...
    def grip(
        ...
        # instruction-word force_percent via _g_axis_force_percent; we do NOT
        # write I2T_PEAK_CURRENT (see sequences.grip docstring — the axis runs
        # with firmware defaults).
        grip_current_amps = 0.3 if grip_lid else 0.2
        # farthest = target_mm + 4.0 mm. Convert 4mm to normalized units
        # because GripParams works in normalized axis frame.
    ...
    def open_gripper(self, position: float | None = None) -> None:
        ...
        # Default to OPEN_GRIPPER_POSITION (0.0 mm) — the open position
        # PickPlaceTask expects. Driving G toward effective_software_min
        # (-7.0 mm) is too aggressive and
    ...
    def jog(self, params: BaseJogParams) -> float:
        ...
        final_normalized = sequences.jog(
            ...
            sequences.JogParams(
                ...
                velocity_limit=limits.velocity,
                acceleration_limit=limits.acceleration,
                # The "exceeded destination" check uses a 0.05 mm epsilon
                # near the farthest point. Convert to normalized axis units
    ...
    #     node — i.e. the G axis address (node=6, dev=0).
    #   * Enable:  SET   subcmd=76 val=2
    #     Disable: SET   subcmd=76 val=0
    #     Read:    GET   subcmd=76 → uint; bit 0 = plate present
    #   * There is also a master-node "enable plate-presence sensor" property,
    #     but setting it has no effect on this firmware (same pattern as the
    #     axis peak-current write — see sequences.jog docstring). We therefore
    #     skip it: only the G-device SET subcmd=76 has actual wire effect, and
    #     scans succeed without it.
    ...
    def read_plate_sensor(self, transient_ms: int = 0) -> bool:
        ...
        sensor failed — an unreadable sensor must never be reported as
        "no plate".
        """
    ...
    def scan_stack_with_gripper(
        ...
        *,
        ...
        Returns a dict in the shape ScanStackHeightTask._scan_with_plate_sensor
        consumes:
        ...
        # Cap acceleration at 80 mm/s²; fall back to 40 if the axis's limit
        # is non-positive.
    ...
    def get_all_positions(self) -> dict[str, float]:
        ...
        Naive per-axis loop — the Gemini protocol has no multipacket read for
        position queries.
        """
        out: dict[str, float] = {}
    ...
    def _state_flags(self) -> int:
        """State bitfield: 0x01=E-stop, 0x02=motor_power (any axis enabled),
        0x04=go button.

        Go-button state is exposed as a master-node property for which we
        don't have a verified on-wire subcommand mapping. For now the bit
        stays 0 — the Go button is an operator-advance input, not a motion
    ...
    def _axis_telemetry(self) -> dict[str, dict[str, Any]]:
        ...
        omitted rather than faked — consumers (tasks.py ``_fmt_telemetry``)
        use ``isinstance`` checks on each field, so missing keys are fine.
        """
    ...
    def get_state_snapshot(self, max_age_s: float = 0.15) -> dict[str, Any]:
        ...
        Cached for ``max_age_s`` seconds so rapid callers (web UI poll,
        tasks.py logging) don't hammer the wire.
        """
        now = time.monotonic()
        if (
    ...
    def send_command(
        ...
        no-op or to an equivalent native method.
        """
        ...
        if command_id == CommandID.CLEAR_MOTOR_POWER_FAULT:
            ...
            # reachable over Gemini, so there is nothing to clear.

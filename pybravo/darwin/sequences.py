...
- ``jog`` validates its final position against a tolerance window, reporting
  "exceeded destination" and "unable to reach destination" separately.
- ``scan_stack_with_gripper`` is not yet implemented; it requires plate-sensor
  serial subcommands.
"""
...
def _convert_mm_to_percent(value_mm: float, limit_mm: float) -> float:
    ...
    Returns 100.0 if the limit is unknown or non-positive.
    """
...
def _g_axis_force_percent(grip_current_amps: float) -> float:
    ...
    scaled by 80/30. The input is the axis-side peak current in **amps**, not
    a 0-1 fraction.
    """
...
def _z_axis_force_percent(peak_current_amps: float) -> float:
    ...
    # Anchor points (amps, force_percent).
...
def grip(
    ...
    *,
    ...
    We therefore do **not** write ``I2T_PEAK_CURRENT`` here — writing an
    alternative peak both (a) can fail with OUT_OF_RANGE on the G axis on some
    firmware states and (b) means the ``finally`` restore to a cached original
    can NAK and mask the real error.
...
class OpenGripperParams:
    ...
    velocity_mm: float = 60.0         # default max velocity in mm/s
...
def open_gripper(
    ...
    *,
    ...
    inst = Instruction(
        ...
        velocity_percent=velocity_pct,
        acceleration_percent=acceleration_pct,
        # jerk_percent=0.0 historically meant "default", which clamps 0 → 100.
...
class JogParams:
    ...
    # The check is defined as 0.05 mm; callers should divide by the axis's
...
def jog(
    ...
    *,
    ...
    This path emits **zero** param-DB writes to the axis: neither the peak
    current nor the position-error-max is manipulated. The firmware defaults
    for I2T_PEAK_CURRENT and POS_ERR_LIMIT remain in force, and the jog's
    force control is done entirely via the ``force_percent`` bits of the
    instruction word. Validated over 4 successful Tips-On bench cycles.
